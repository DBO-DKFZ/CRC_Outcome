...
import TtoE_SLP_v3 as TtoE_SLP
...
def ablation_studies(args):
    ci_strings, ibs_strings, test_sizes, experiments = [], [], [], [] 
    
     # feature extractor
        
    for feature_column in ['rand_features', 'image_features', 'cam_features', 'sub_features', 'ciga_features', 'retccl_features', 'r26_features','dino_features', 'dino_tcga_features']:
        args.exp_name='OS_v2_test2'
        args.feature_column = feature_column
        args.num_durations = 60
        mean_ci, ci_125, ci_975, mean_ibs, ibs_125, ibs_975, test_size = test_TtoE_ensemble(args)
        ci_strings.append(f'{mean_ci}, ({ci_125}-{ci_975})')
        ibs_strings.append(f'{mean_ibs}, ({ibs_125}-{ibs_975})')
        test_sizes.append(test_size)
        experiments.append(feature_column)
    
    # tissue type 
    for tissue in ['STR', 'TUM+STR', 'ALL', 'other']:
        args.exp_name='OS_v2_test2'
        args.feature_column = 'dino_features'
        args.tissue_type = tissue
        args.num_durations = 60
        mean_ci, ci_125, ci_975, mean_ibs, ibs_125, ibs_975, test_size = test_TtoE_ensemble(args)
        ci_strings.append(f'{mean_ci}, ({ci_125}-{ci_975})')
        ibs_strings.append(f'{mean_ibs}, ({ibs_125}-{ibs_975})')
        test_sizes.append(test_size)
        experiments.append(tissue)
    
       
        #output neurons
    for outputs in [1,5,10,30]:
        args.exp_name = 'OS_v2_test2_Outputneurons_'+str(outputs)
        args.feature_column = 'dino_features'
        args.tissue_type = 'TUM'
        args.num_durations = outputs 
        mean_ci, ci_125, ci_975, mean_ibs, ibs_125, ibs_975, test_size = test_TtoE_ensemble(args)
        ci_strings.append(f'{mean_ci}, ({ci_125}-{ci_975})')
        ibs_strings.append(f'{mean_ibs}, ({ibs_125}-{ibs_975})')
        test_sizes.append(test_size)
        experiments.append(str(outputs))

        
        
    frame = pd.DataFrame({'Exp':experiments, 
                         'N': test_sizes,
                         'C-index': ci_strings,
                         'IBS': ibs_strings})
  
    
    frame.to_pickle('/home/ext_julia/pipeline/results/ablation_studies.pkl')
   
    

def test_TtoE_ensemble(args):
    print(args.feature_column)
    # graz set: 
    ...
    df_test[args.duration_column] = df_test[args.duration_column] # to do: check 
    
    # tissue types: args.tissue_type... TUM, STR, TUM+STR, other, ALL 
    # to Do: checken ob für alle patienten mind. 1 tile vorhanden ist
    ...
    if args.num_durations !=60:
            ...
            print(devider)
            df_train['time_curated_v3'] = [math.ceil(time/devider) for time in df_train[args.duration_column]]
            df_val['time_curated_v3'] = [math.ceil(time/devider) for time in df_val[args.duration_column]]
            df_test['time_curated_v3'] = [math.ceil(time/devider) for time in df_test[args.duration_column]]
    if args.num_durations !=60:
        time_column = 'time_curated_v3'
    else:
    ...
    if args.save == True:
        ...
        df_test.to_pickle('/home/ext_julia/pipeline/results/preds_'+ args.test + '_'+args.feature_column+'_'+args.subgroup+'_'+args.test_subgroup+'_'+args.attention+'.pkl')
    ...
    if args.num_durations ==1: 
        ...
        mean_ci = round(ci_last_all,4)
        ci_125 = round(c_index_ci[125],4)
        ci_975 = round(c_index_ci[975],4)
        mean_ibs = '-'
        ibs_125 = '-'
        ibs_975 = '-'
        return mean_ci, ci_125, ci_975, mean_ibs, ibs_125, ibs_975, len(df_test)
    else:
        ...
        mean_ci = round(c_index,4)
        ci_125 = round(c_index_ci[125],4)
        ci_975 = round(c_index_ci[975],4)
        mean_ibs = round(ibs,4)
        ibs_125 = round(ibs_ci[125],4)
        ibs_975 = round(ibs_ci[975],4)
        return mean_ci, ci_125, ci_975, mean_ibs, ibs_125, ibs_975, len(df_test)
